...
# ══════════════════════════════════════════════════════════════════════════════
# CSS TOÀN CỤC – GIAO DIỆN TRẺ EM VUI NHỘN (NÂNG CẤP)
# ══════════════════════════════════════════════════════════════════════════════
st.markdown("""
<style>
...
@keyframes floatUp {
    0%   { transform: translateY(110vh) rotate(0deg); opacity: 0.12; }
...
.hud-bar {
    ...
    padding: 10px 18px;
    margin-bottom: 16px;
    box-shadow: 0 6px 24px rgba(26,26,46,0.3), 0 0 0 2px rgba(255,255,255,0.05);
    gap: 8px;
    flex-wrap: wrap;
...
.subject-card-math {
    ...
    text-align: center;
    margin-bottom: 10px;
    box-shadow: 0 10px 28px rgba(255,217,61,0.35);
...
div.stButton > button[kind="primary"]:hover {
    ...
    box-shadow: 0 12px 28px rgba(255,107,107,0.55) !important;
}
...
div.stButton > button[kind="secondary"]:hover {
    ...
    box-shadow: 0 6px 14px rgba(168,85,247,0.18) !important;
}
...
def save_to_google_sheet(data: dict):
    """
    Lưu kết quả vào Google Sheets qua gspread.
    data = { name, subject, topic, score, total, points, level, timestamp, badge }
    Cần cấu hình service account JSON trong st.secrets["gcp_service_account"]
    """
    try:
        import gspread
        ...
        if creds_dict is None:
            return  # Chưa cấu hình secrets → bỏ qua
        ...
        pass  # Không crash app khi Google Sheets lỗi
...
def init_state():
    defaults = {
        ...
        "streak":        0,
        "best_streak":   0,
        # Badge history (list of dicts)
        "badge_history": [],
        # Session leaderboard
        "leaderboard":   [],
        # Bonus awarded this answer
        "bonus_this":    0,
    }
...
def get_badge_for_result(score: int, total: int, streak: int) -> dict:
    """Trả về dict { key, label, css_class } của huy hiệu dựa theo kết quả."""
...
def render_sound(correct: bool):
    if correct:
        js = """<script>(function(){
            ...
            function tone(f,t,d,type){var o=c.createOscillator(),g=c.createGain();
            o.connect(g);g.connect(c.destination);o.type=type||'sine';
            o.frequency.value=f;g.gain.setValueAtTime(0.3,t);
            g.gain.exponentialRampToValueAtTime(0.001,t+d);o.start(t);o.stop(t+d);}
            tone(880,c.currentTime,0.15);
            tone(1100,c.currentTime+0.15,0.15);
            tone(1320,c.currentTime+0.3,0.3);
        })();</script>"""
    else:
        js = """<script>(function(){
            var c=new(window.AudioContext||window.webkitAudioContext)();
...
def render_confetti():
    ...
    for i in range(40):
        ...
        pieces += (f'<div class="confetti-piece" style="left:{lft}%;top:-20px;'
                   f'background:{c};width:{sz}px;height:{sz}px;'
    ...
    if lb:
        ...
        for i, entry in enumerate(lb[:5]):
            ...
            <div class="lb-row">
                ...
                <span style="font-size:13px;color:#9ca3af;">{entry['badge']}</span>
            </div>"""
        st.markdown(f"""
        <div class="leaderboard-box">
            <div class="leaderboard-title">🏆 Bảng Xếp Hạng Phiên</div>
            {rows_html}
        </div>""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        f"<p style='font-size:16px;font-weight:800;color:#7c3aed;text-transform:uppercase;"
        f"letter-spacing:.06em;margin:14px 0 14px;position:relative;z-index:1;'>"
        f"{sub['icon']} {sub['label']} — Chọn chủ đề</p>",
        unsafe_allow_html=True
    )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div class="progress-outer">
        <div class="progress-inner" style="width:{pct}%;">
    ...
    if not answered and streak >= 3:
        ...
        st.markdown(
            f'<div class="streak-banner">{fire} Chuỗi đúng {streak} câu! Siêu đỉnh!</div>',
            unsafe_allow_html=True
        )
    ...
    if answered:
        if selected == q["ans"]:
            ...
            st.balloons()
            # Bonus banner
            bonus = st.session_state.bonus_this
            if bonus:
                st.markdown(
                    f'<div class="bonus-banner">🎁 STREAK BONUS +{bonus} điểm! Tuyệt vời!</div>',
                    unsafe_allow_html=True
                )
            st.markdown(f"""
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # Xác định mascot + thông điệp
    if badge["key"] == "superstar":
        mascot = MASCOTS["superstar"]
        title  = "🔥 Siêu Sao Hoàn Hảo!"
        msg    = f"Xuất sắc {name}! 10/10 câu đúng + chuỗi {bstrk}! Bạn là thiên tài! 🚀"
        render_celebration_sound()
        render_confetti()
        st.balloons()
    elif badge["key"] == "gold":
    ...
    <div class="total-pts-box">
        ...
        <div>
            ...
            <div class="total-pts-lbl">Tổng điểm · Level {lv} · Streak tốt nhất: {bstrk}</div>
        </div>
        <span class="total-pts-icon">🎮</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    if len(bh) > 1:
        ...
        st.markdown(f"""
        <div class="badge-history-box">
            <div class="badge-history-title">🎖️ Huy Hiệu Đã Đạt</div>
            <div class="badge-history-row">{chips}</div>
        </div>""", unsafe_allow_html=True)
